...
class TemporalSimilarityLoss(nn.Module):
    ...
    def forward(self, pred, patient_batch_indices, timestamps):
        ...
        unique_pids = torch.unique(patient_batch_indices)
        print(f"Batch size: {batch_size}, Unique patients: {len(unique_pids)}")
        for pid in unique_pids[:3]:  # Just first 3 patients
            count = (patient_batch_indices == pid).sum().item()
            print(f"  Patient {pid.item()}: {count} rows")
...
def evaluate_trajectory_fidelity(df, imputed_dfs, columns_to_impute, 
                                 ...
                                 n_patients_sample=200):  # ← Reduced from 100 (when it is too small, Da Xu not work...)
    """
...
def main_real_data(df, columns_to_impute):
    ...
    trajectory_results = evaluate_trajectory_fidelity(
        df, imputed_dfs, columns_to_impute, n_patients_sample=200  # ← fast
    )
