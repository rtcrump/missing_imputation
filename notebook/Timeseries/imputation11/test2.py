...
class LSTMImputer(nn.Module):
    ...
    def __init__(self, input_dim, output_dim, hidden_dim=64, n_layers=1):
        ...
        self.output_layer = nn.Linear(hidden_dim * 2, output_dim)  # *2 for bidir
...
def apply_lstm_imputation(df, columns_to_impute,
                          ...
                          hidden_dim=64, n_layers=1, epochs=50,
...
def apply_mice_longitudinal_imputation(df, columns_to_impute,
    ...
    def _add_temporal_features(frame, fit_reference=None):
        ...
        for col_name in columns_to_impute:
            lag_name = f'_lag1_{col_name}'
            out[lag_name] = out.groupby(patient_col)[col_name].shift(1)
