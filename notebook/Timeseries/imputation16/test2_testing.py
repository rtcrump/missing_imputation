...
"""
Gemma 1B LLM-based imputation for FACT-E ordinal variables.
...
Pipeline (mirrors Yousif et al. EMBC 2026):
    ...
    4. Fine-tune Gemma-1B-IT with LoRA on (prompt -> integer) pairs
...
def apply_gemma_imputation(df, columns_to_impute,
                           ...
                           model_name='/home/yjkweon2/models/gemma-2-2b-it',
                           seed=42):
    """
    Fine-tune Gemma with LoRA for FACT-E ordinal imputation.
    ...
    model_name : str
        ...
        For true 1B: 'google/gemma-3-1b-it'
    seed : int
...
def evaluate_real_data_imputation(df, columns_to_impute):
    ...
    methods = {
        ...
        'Gemma LLM': apply_gemma_imputation 
    }
        ...
        for fold in range(min(n_folds, n_observed // fold_size)):
            ...
            for method_name, method_func in methods.items():
                ...
                try:
                    ...
                    # For methods other than LSTM and Longitudinal MICE, drop the qol_date column
                    if method_name not in ('LSTM', 'Longitudinal MICE') and 'qol_date' in method_df.columns:
                        method_df = method_df.drop(columns=['qol_date'])
...
def evaluate_trajectory_fidelity(df, imputed_dfs, columns_to_impute, 
                                 ...
                                 n_patients_sample=30):  # ← Reduced from 100 (when it is too small, Da Xu not work...)
    """
...
def plot_longitudinal_metrics(trajectory_results, smoothness_results, pattern_results):
    ...
    if trajectory_results:
        ...
        # FIX: Set y-axis limits to include negative correlations
        min_corr = min(traj_corrs)
        max_corr = max(traj_corrs)
        margin = 0.1
        ax2.set_ylim(min(min_corr - margin, -0.1), max_corr + margin)
    ...
    if smoothness_results:
        ...
        y_min = min_val * 0.95  # e.g., 0.895 * 0.90 = 0.8055
...
def main_real_data(df, columns_to_impute):
    ...
    trajectory_results = evaluate_trajectory_fidelity(
        df, imputed_dfs, columns_to_impute, n_patients_sample=30  # ← fast
    )
