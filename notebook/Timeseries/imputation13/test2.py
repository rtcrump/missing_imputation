...
class TemporalSimilarityLoss(nn.Module):
    """
    Temporal smoothness loss for consecutive timepoints within same patient.
    """
    def __init__(self):
    ...
    def forward(self, pred, patient_batch_indices, timestamps):
        """
        Calculate temporal smoothness loss for consecutive timepoints only.
        
        Parameters
        -----------
        pred : torch.Tensor [batch_size, n_features]
        patient_batch_indices : torch.Tensor [batch_size]
        ...
        # Keep everything on device for backprop
        loss = torch.tensor(0.0, device=device, requires_grad=True)
        pair_count = 0
        
        # Get unique patients
        unique_patients = torch.unique(patient_batch_indices)
        
        for pid in unique_patients:
            # Get indices for this patient
            pid_mask = patient_batch_indices == pid
            pid_indices = torch.where(pid_mask)[0]
            
            if len(pid_indices) < 2:
                continue
            
            # Sort by time
            pid_times = timestamps[pid_indices]
            sorted_order = torch.argsort(pid_times)
            sorted_indices = pid_indices[sorted_order]
            
            # Compute loss for consecutive pairs
            for i in range(len(sorted_indices) - 1):
                idx1 = sorted_indices[i]
                idx2 = sorted_indices[i + 1]
                
                # L2 distance between consecutive predictions
                diff = torch.sum((pred[idx1] - pred[idx2]) ** 2)
                loss = loss + diff
                pair_count += 1
...
def evaluate_trajectory_fidelity(df, imputed_dfs, columns_to_impute, 
                                 ...
                                 n_patients_sample=100):  # ← Reduced from 100
    """
...
def main_real_data(df, columns_to_impute):
    ...
    trajectory_results = evaluate_trajectory_fidelity(
        df, imputed_dfs, columns_to_impute, n_patients_sample=100  # ← fast
    )
