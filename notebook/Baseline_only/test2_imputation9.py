...
def plot_qwk_results(validation_results):
    methods = [m for m in validation_results if 'qwk' in validation_results[m] and validation_results[m]['qwk']]
    means = [np.mean(validation_results[m]['qwk']) for m in methods]
    stds = [np.std(validation_results[m]['qwk']) for m in methods]

    fig, ax = plt.subplots(figsize=(8, 5))
    ax.bar(methods, means, yerr=stds, capsize=5, alpha=0.7, color='steelblue')
    ax.set_title('Quadratic Weighted Kappa by Method (Higher = Better)')
    ax.set_ylabel('QWK')
    ax.set_ylim([-1, 1])
